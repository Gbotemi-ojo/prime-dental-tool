// src/pages/PatientReceiptsPage.jsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { toast } from 'react-toastify';
import './patient-receipts-page.css'; // New CSS file for this page
...
export default function PatientReceiptsPage() {
  ...
  // Hardcoded list of HMOs
  const hmoOptions = [
    { name: "IHMS", status: "ONBOARD" },
    { name: "HEALTH PARTNERS", status: "ONBOARD" },
    { name: "ZENOR", status: "NOT ONBOARD" },
    { name: "PHILIPS", status: "ONBOARD" },
    { name: "PRO HEALTH", status: "NOT ONBOARD" },
    { name: "FOUNTAIN HEALTH", status: "NOT ONBOARD" },
    { name: "DOT HMO", status: "NOT ONBOARD" },
    { name: "CLEARLINE", status: "ONBOARD" },
    { name: "STERLING HEALTH", status: "NOT ONBOARD" },
    { name: "OCEANIC", status: "NOT ONBOARD" },
    { name: "SUNU", status: "NOT ONBOARD" },
    { name: "LIFEWORTH", status: "NOT ONBOARD" },
    { name: "CKLINE", status: "NOT ONBOARD" },
    { name: "WELLNESS", status: "ONBOARD" },
    { name: "RELIANCE", status: "ONBOARD" },
    { name: "FIRST GUARANTEE", status: "NOT ONBOARD" },
    { name: "THT", status: "NOT ONBOARD" },
    { name: "DOHEEC", status: "NOT ONBOARD" },
    { name: "GNI", status: "NOT ONBOARD" },
    { name: "MH", status: "NOT ONBOARD" },
    { name: "AIICO MULTISHIELD", status: "NOT ONBOARD" },
    { name: "GREENBAY", status: "NOT ONBOARD" },
    { name: "MARINA", status: "NOT ONBOARD" },
    { name: "EAGLE", status: "NOT ONBOARD" },
    { name: "MEDIPLAN", status: "NOT ONBOARD" },
    { name: "METROHEALTH", status: "ONBOARD" },
    { name: "RONSBERGER", status: "NOT ONBOARD" },
    { name: "WELPRO", status: "NOT ONBOARD" },
    { name: "GORAH", status: "NOT ONBOARD" },
    { name: "SMATHEALTH", status: "NOT ONBOARD" },
    { name: "AXA MANSARD", status: "NOT ONBOARD" },
    ...
    { name: "LEADWAY", status: "ONBOARD" }, // Duplicate, but kept as per list
    { name: "QUEST", status: "ONBOARD" },
    { name: "AVON", status: "ONBOARD" }, // Duplicate, but kept as per list
    { name: "CLEARLINE", status: "ONBOARD" } // Duplicate, but kept as per list
  ];
  ...
  useEffect(() => {
    const fetchPatientDetails = async () => {
      ...
      if (!token || (role !== 'owner' && role !== 'staff')) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setError(errorData.error || `Failed to fetch patient details. Status: ${response.status}`);
        }
      } catch (err) {
  ...
  const handleHmoChange = (e) => {
    ...
    // Reset covered amount if a non-onboard HMO is selected or no HMO
    const hmo = hmoOptions.find(h => h.name === selectedHmoName);
    if (!hmo || hmo.status === "NOT ONBOARD") {
      setHmoCoveredAmount('');
    }
  };
      ...
      <header className="receipts-header">
        ...
        <div className="actions">
          ...
          <button onClick={handleGenerateReceipt} className="generate-receipt-button">
            <i className="fas fa-file-invoice"></i> Generate Receipt
          </button>
          {showReceipt && (
      ...
      {!showReceipt ? (
        <section className="receipt-form-section">
          ...
          <div className="hmo-selection">
            ...
            {selectedHMO && !isHmoOnboard && (
              <p className="hmo-status-message not-onboard">Selected HMO is NOT ONBOARD. Patient will pay full amount.</p>
            )}
            {isHmoOnboard && (
        ...
        <div className="receipt-display-area printable-content">
          ...
          <div className="receipt-summary">
            ...
            {selectedHMO && (
              <p>
                <strong>HMO ({selectedHMO}):</strong> {isHmoOnboard ? `₦${coveredAmount.toLocaleString()} Covered` : 'Not Onboard'}
              </p>
            )}
